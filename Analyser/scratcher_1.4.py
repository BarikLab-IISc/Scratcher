...
class ScratcherGUI:
    ...
    def create_detect_tab(self):
        ...
        # Variables initialization fix for the new variable
    ...
    def create_analyse_tab(self):
        ...
        analyses = [
            "Itch Bout Frequency",  # New feature added
            "Peak scratching duration",
            "Latency to itch onset", 
            "Area under the curve (scratching over time)",
            "Slope of scratching session (rate of increase or decrease)",
    ...
    def start_detection(self):
        ...
        # Integranting YOLO Detection logic
        model_path = getattr(self, 'custom_model_path', tk.StringVar(value="best.pt")).get()
    ...
    def start_analysis(self):
        ...
        if "Slope of scratching session (rate of increase or decrease)" in selected_analyses:
            try:
                ...
                input_dir = self.analysis_input_folder.get()
                xlsx_files = [f for f in os.listdir(input_dir) if f.endswith('.xlsx') and not f.startswith('raster')]
                if xlsx_files:
                    file_path = os.path.join(input_dir, xlsx_files[0]) # just picking the first for the demo
                    output_path = os.path.join(self.analysis_output_folder.get(), f"slope_plot_{xlsx_files[0].replace('.xlsx', '.png')}")
                    ...
                    results_msg.append("Slope Plotting: No suitable Excel file found in input directory.")
            except Exception as e:
                results_msg.append(f"Slope Plotting Error: {e}")
        
        unimplemented = [a for a in selected_analyses if a not in ["Itch Bout Frequency", "Slope of scratching session (rate of increase or decrease)"]]
        if unimplemented:
