...
DOCS_URL = "https://github.com/BarikLab-IISc/Scratcher/blob/main/Step-wise%20guide%20to%20use%20Scratcher.pdf"  # <-- replace with real docs
...
def validate_roboflow_dataset(dataset_dir):
    ...
    return all(os.path.exists(os.path.join(dataset_dir, r)) for r in required)
...
# ---------------- YOLO PREDICTION TAB LOGIC ---------------- #

def run_yolo():
    model = YOLO(model_entry_predict.get())
    start = time.time()

    if dir_entry.get():
        os.chdir(dir_entry.get())
    ...
    result_label.config(text=f"Completed in {time.time() - start:.2f} seconds")
...
def start_training_safe():
    ...
    messagebox.showinfo(
        "Training Complete",
        f"Training finished in {time.time() - start:.2f} seconds"
    )
...
tk.Label(analyser_frame, text="YOLO Model File").grid(row=1, column=0)
model_entry = tk.Entry(analyser_frame, width=50)
model_entry.grid(row=1, column=1)
tk.Button(analyser_frame, text="Browse", command=lambda: browse_file(model_entry)).grid(row=1, column=2)

tk.Label(analyser_frame, text="Input Folder").grid(row=2, column=0)
input_folder_entry = tk.Entry(analyser_frame, width=50)
input_folder_entry.grid(row=2, column=1)
tk.Button(analyser_frame, text="Browse", command=lambda: browse_directory(input_folder_entry)).grid(row=2, column=2)

tk.Label(analyser_frame, text="Output Folder").grid(row=3, column=0)
output_folder_entry = tk.Entry(analyser_frame, width=50)
output_folder_entry.grid(row=3, column=1)
tk.Button(analyser_frame, text="Browse", command=lambda: browse_directory(output_folder_entry)).grid(row=3, column=2)

tk.Label(analyser_frame, text="Confidence Threshold").grid(row=4, column=0)
...
tk.Label(predict_frame, text="Model Path").grid(row=1, column=0)
model_entry_predict = tk.Entry(predict_frame, width=50)
model_entry_predict.grid(row=1, column=1)
tk.Button(predict_frame, text="Browse", command=lambda: browse_file(model_entry_predict)).grid(row=1, column=2)

tk.Label(predict_frame, text="Input Video").grid(row=2, column=0)
input_entry = tk.Entry(predict_frame, width=50)
input_entry.grid(row=2, column=1)
tk.Button(predict_frame, text="Browse", command=lambda: browse_file(input_entry)).grid(row=2, column=2)

tk.Label(predict_frame, text="Change Directory").grid(row=3, column=0)
dir_entry = tk.Entry(predict_frame, width=50)
dir_entry.grid(row=3, column=1)
tk.Button(predict_frame, text="Browse", command=lambda: browse_directory(dir_entry)).grid(row=3, column=2)

save_var = tk.BooleanVar(value=True)
tk.Checkbutton(predict_frame, text="Save Output", variable=save_var).grid(row=4, column=0)

tk.Label(predict_frame, text="Confidence").grid(row=5, column=0)
conf_entry = tk.Entry(predict_frame, width=10)
conf_entry.insert(0, "0.5")
conf_entry.grid(row=5, column=1, sticky="w")

tk.Button(predict_frame, text="RUN YOLO", command=run_yolo).grid(row=6, column=1, pady=20)
result_label = tk.Label(predict_frame, text="")
...
tk.Button(
    train_frame,
    text="📘 Training Docs & Roboflow Guide",
    command=open_docs
).grid(row=0, column=2, sticky="e", padx=10, pady=10)

tk.Label(train_frame, text="Train YOLO Model (Simple Mode)", font=("Arial", 14, "bold")).grid(
    row=1, column=0, columnspan=3, pady=10
)

tk.Label(train_frame, text="Step 1: Create Dataset using Roboflow", font=("Arial", 11, "bold")).grid(
    row=2, column=0, sticky="w", padx=10
)
tk.Button(train_frame, text="Open Roboflow Website", command=open_roboflow).grid(
    row=2, column=1, sticky="w"
)

tk.Label(train_frame, text="Step 2: Select Roboflow YOLO Dataset Folder", font=("Arial", 11, "bold")).grid(
    row=3, column=0, sticky="w", padx=10, pady=(10, 0)
)

dataset_entry = tk.Entry(train_frame, width=50)
dataset_entry.grid(row=4, column=0, columnspan=2, padx=10)
tk.Button(train_frame, text="Browse", command=lambda: browse_directory(dataset_entry)).grid(row=4, column=2)

tk.Label(train_frame, text="(Folder must contain train/, valid/, data.yaml)", fg="gray").grid(
    row=5, column=0, columnspan=3, padx=10, sticky="w"
)

tk.Label(train_frame, text="Step 3: Select Pretrained YOLO Model", font=("Arial", 11, "bold")).grid(
    row=6, column=0, sticky="w", padx=10, pady=(10, 0)
)

model_entry_train = tk.Entry(train_frame, width=50)
model_entry_train.grid(row=7, column=0, columnspan=2, padx=10)
tk.Button(train_frame, text="Browse", command=lambda: browse_file(model_entry_train)).grid(row=7, column=2)

tk.Label(train_frame, text="Step 4: Training Epochs", font=("Arial", 11, "bold")).grid(
    row=8, column=0, sticky="w", padx=10, pady=(10, 0)
)

epochs_entry = tk.Entry(train_frame, width=10)
epochs_entry.insert(0, "50")
epochs_entry.grid(row=9, column=0, padx=10, sticky="w")
...
tk.Button(
    ...
    font=("Arial", 12, "bold"),
    height=2
).grid(row=10, column=0, columnspan=3, pady=30)
